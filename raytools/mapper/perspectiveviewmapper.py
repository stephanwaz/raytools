...
class PerspectiveViewMapper(ViewMapper):
    """translate between world direction vectors and normalized UV space for a
    given view angle. pixel projection yields perpespective projection
    ...
    vh: float, optional
        horizontal view angle limited to 180
    vv: float, optional
        vertical view angle limited to 180
    """

    def __init__(self, dxyz=(0.0, 1.0, 0.0), vv=100.0, vh=100.0, name='view',
                 origin=(0, 0, 0), jitterrate=0.5):
        viewangle = max(vh, vv)
        if viewangle >= 180:
            raise ValueError("View angle for PerspectiveViewMapper must be less than 180")
